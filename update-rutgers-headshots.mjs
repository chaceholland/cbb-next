/**
 * Scrape and update Rutgers pitcher headshots from their official roster page
 */
...
const RUTGERS_ROSTER_URL = 'https://scarletknights.com/sports/baseball/roster';
const RUTGERS_TEAM_ID = '102';

async function scrapeHeadshots() {
  console.log('🔍 Scraping Rutgers headshots...\n');

  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();

  try {
    await page.goto(RUTGERS_ROSTER_URL, { waitUntil: 'load', timeout: 30000 });
    ...
    const scraped = await page.evaluate(() => {
      ...
      allImages.forEach(img => {
        ...
        // Check for various Rutgers image CDN patterns
        const isRutgersImage = src.includes('scarletknights.com') ||
    ...
    console.log(`📸 Matched ${headshotList.length} player headshots on Rutgers roster\n`);

    // Get Rutgers pitchers from database
    const { data: dbPitchers, error: dbError } = await supabase
      .from('cbb_pitchers')
      .select('pitcher_id, name, display_name, headshot')
      .eq('team_id', RUTGERS_TEAM_ID);

    if (dbError) {
      console.error('❌ Error fetching Rutgers pitchers:', dbError.message);
      return 0;
    }

    console.log(`🎯 Found ${dbPitchers.length} Rutgers pitchers in database\n`);
    ...
    console.error('❌ Error scraping Rutgers headshots:', error);
    return 0;
  } finally {
    await browser.close();
  }
...
async function main() {
  console.log('🏀 Rutgers Headshot Update\n');
  console.log('='.repeat(50) + '\n');

  const headshotsUpdated = await scrapeHeadshots();

  console.log('\n' + '='.repeat(50));
  console.log(`\n✅ Complete! ${headshotsUpdated} Rutgers pitcher headshots updated`);
