...
export function ScheduleView() {
  ...
  const watchOrder = filters.watchOrder as
    ...
    | "upcoming"
    | "favorites";
  const pitcherFilter = filters.pitcherFilter as
    | "favorites-or-played"
    | "favorites-only"
    | "played-only"
    | "all";
  const selectedWeeks = useMemo(
    () => new Set(filters.selectedWeeks as number[]),
  ...
  const setTeamSearch = useCallback(
    (search: string) => {
  ...
  const setWatchOrder = useCallback(
    (
      order:
        | "all"
        | "unwatched"
  ...
  const setPitcherFilter = useCallback(
    (
      filter: "favorites-or-played" | "favorites-only" | "played-only" | "all",
    ) => {
  ...
  const [pitcherDataQualityIssues, setPitcherDataQualityIssues] =
    useLocalStorage<PitcherDataQualityIssue[]>(
      "cbb-pitcher-data-quality-issues",
      [],
    );
  const [gameDataQualityIssues, setGameDataQualityIssues] = useLocalStorage<
    GameDataQualityIssue[]
  >("cbb-game-data-quality-issues", []);
  const [showIssuesOnly, setShowIssuesOnly] = useState(false);
  const [showLiveDataOnly, setShowLiveDataOnly] = useState(false);
      ...
      try {
        ...
        allPitchers.forEach((p) => {
          if (p.headshot) {
            ...
            hMap[`${p.team_id}:${norm}`] = p.headshot;
            // Last-name-only key for D1Baseball partial names (e.g. "414:ln:smith")
            // Mark as null on collision so we don't misassign
        ...
        const computeWeek = (dateStr: string) => {
          ...
          return Math.max(
            1,
            Math.floor(
              (d.getTime() - seasonStart.getTime()) / (1000 * 60 * 60 * 24 * 7),
            ) + 1,
          );
        };
        const weeks = [
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        setFavoriteTeamIds(
          new Set((data || []).map((p: { team_id: string }) => p.team_id)),
        );
      });
  }, [favorites]);
  ...
  const fetchWeekParticipation = useCallback(
    async (week: number, weekGames: CbbGame[]) => {
      if (loadedWeeks.has(week) || loadingWeeks.has(week)) return;

      const completedGameIds = weekGames
  ...
  const getGameConference = useCallback(
    (g: CbbGame): string => {
  ...
  const handleCopyIssues = useCallback(async () => {
    ...
    const combinedReport = [gameIssueReport, pitcherIssueReport]
      .filter(Boolean)
      .join("\n\n");
  ...
  const conferenceCounts = useMemo(() => {
    ...
    games.forEach((g) => {
      ...
      else if (conf.includes("Pac-12") || conf.includes("Pac 12"))
        label = "Pac-12";
      else if (conf.includes("American")) label = "American";
      else if (conf.includes("Sun Belt")) label = "Sun Belt";
      else if (conf.includes("C-USA") || conf.includes("Conference USA"))
        label = "C-USA";
  ...
  const filteredGames = useMemo(() => {
    ...
    if (conferences.size > 0) {
      result = result.filter((g) => {
        ...
        return Array.from(conferences).some((selectedConf) => {
          ...
          if (selectedConf === "Pac-12")
            return conf.includes("Pac-12") || conf.includes("Pac 12");
          if (selectedConf === "American") return conf.includes("American");
          if (selectedConf === "Sun Belt") return conf.includes("Sun Belt");
          if (selectedConf === "C-USA")
            return conf.includes("C-USA") || conf.includes("Conference USA");
          if (selectedConf === "Mountain West")
            return conf.includes("Mountain West");
    ...
    if (teamSearch.trim()) {
      ...
      result = result.filter((g) => {
        const hn = (
          teams[g.home_team_id]?.display_name ||
          g.home_name ||
          ""
        ).toLowerCase();
        const an = (
          teams[g.away_team_id]?.display_name ||
          g.away_name ||
          ""
        ).toLowerCase();
    ...
    // Live data filter - only show games with pitcher participation data
    if (showLiveDataOnly) {
      result = result.filter(
        (g) => (participationByGame[g.game_id] || []).length > 0,
      );
    }
    ...
    Object.entries(gamesByWeek).forEach(([weekStr, weekGames]) => {
      ...
      sortedGames.forEach((game) => {
        ...
        sortedGames.forEach((otherGame) => {
          ...
          const sameTeams =
            (game.home_team_id === otherGame.home_team_id &&
              game.away_team_id === otherGame.away_team_id) ||
            (game.home_team_id === otherGame.away_team_id &&
              game.away_team_id === otherGame.home_team_id);
  ...
  const handleExportCSV = useCallback(() => {
    ...
    filteredGames.forEach((game) => {
      const homeTeam =
        teams[game.home_team_id]?.display_name || game.home_name || "";
      const awayTeam =
        teams[game.away_team_id]?.display_name || game.away_name || "";
      const row = [
        game.date,
        game.week?.toString() || "",
        `"${homeTeam}"`,
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        {/* Live Data Filter */}
        <button
          onClick={() => setShowLiveDataOnly(!showLiveDataOnly)}
          className={cn(
            "flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all duration-200",
            showLiveDataOnly
        ...
        >
          ...
          >
            ...
            <path
              ...
              strokeLinejoin="round"
              strokeWidth={2}
              d="M12 2v2m0 16v2M2 12h2m16 0h2m-3.636-6.364-1.414 1.414M6.05 17.95l-1.414 1.414M17.95 17.95l-1.414-1.414M6.05 6.05 4.636 4.636"
            />
          </svg>
          {showLiveDataOnly ? "Live Data: ON" : "Live Data Only"}
        </button>
        ...
        {(pitcherDataQualityIssues.length > 0 ||
          gameDataQualityIssues.length > 0) && (
          <>
            <button
            ...
            >
              ...
              {showIssuesOnly
                ? "Show All"
                : `Show Issues (${pitcherDataQualityIssues.length + gameDataQualityIssues.length})`}
            </button>
            <button
                      ...
                      gamesByWeek[week].map((game, i) => (
                        ...
                        >
                          <GameCard
                            ...
                            participation={
                              participationByGame[game.game_id] || []
                            }
                            ...
                            onToggleFavorite={() =>
                              toggleFavoriteGame(game.game_id)
                            }
                            onToggleWatched={() =>
                        ...
                        >
                          ...
                          {seriesGames.length > 1 && (
                            <div className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-blue-900/30 to-purple-900/30 rounded-lg border border-blue-800">
                              ...
                              <span className="text-xs text-slate-400">
                                ...
                                vs{" "}
                                {teams[seriesGames[0].home_team_id]
                                  ?.display_name || seriesGames[0].home_name}
                              </span>
                            </div>
                          )}
                          {/* Games in series */}
                          {seriesGames.map((game) => (
                            <GameCard
                              ...
                              onGameIssueToggle={handleGameIssueToggle}
                              onClick={() => setSelectedGame(game)}
                              isFavorite={favoriteGameIds.includes(
                                game.game_id,
                              )}
                              isWatched={watchedGameIds.includes(game.game_id)}
                              onToggleFavorite={() =>
