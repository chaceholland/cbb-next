#!/usr/bin/env node
/**
 * Add Dallas Baptist to cbb_teams + scrape 2026 roster + insert games
 * Team ID: 263, Conference: Independent
 */
...
const TEAM = {
  ...
  display_name: "Dallas Baptist",
  conference: "Independent",
  logo: "/logos/263.png",
};
const ROSTER_URL = "https://dbupatriots.com/sports/baseball/roster";
...
async function upsertTeam() {
  ...
  console.log("✅ Dallas Baptist added to cbb_teams\n");
}
...
async function insertPitchers(pitchers) {
  await supabase.from("cbb_pitchers").delete().eq("team_id", "263");
  if (!pitchers.length) {
    console.log("⚠️  No pitchers to insert.\n");
    return;
  }
  const records = pitchers.map((p, i) => ({
    pitcher_id: `263-P${i + 1}`,
    team_id: "263",
    name: p.name,
...
async function insertGames() {
  ...
  const dbuGames = all.filter(
    (g) =>
      String(g.home_team_id || g.home || "") === "263" ||
      String(g.away_team_id || g.away || "") === "263",
  );
  console.log(
    `\n📋 Found ${dbuGames.length} Dallas Baptist games in schedule.json\n`,
  );

  const records = dbuGames
    .map((g) => ({
...
// ── Main ───────────────────────────────────────────────────────────────────────
async function main() {
  console.log("⚾ Adding Dallas Baptist to CBB Pitcher Tracker");
  ...
  console.log("\n" + "=".repeat(60));
  console.log("✅ Dallas Baptist fully added to Rosters + Schedule.");
  console.log("=".repeat(60));
}
