/**
 * Scrape and update Alabama pitcher headshots from their official roster page
 */
...
const ALABAMA_ROSTER_URL = 'https://rolltide.com/sports/baseball/roster';
const ALABAMA_TEAM_ID = '148';

async function scrapeHeadshots() {
  console.log('🔍 Scraping Alabama headshots...\n');

  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();

  try {
    await page.goto(ALABAMA_ROSTER_URL, { waitUntil: 'load', timeout: 30000 });
    ...
    const scraped = await page.evaluate(() => {
      ...
      allImages.forEach(img => {
        ...
        // Check for Alabama image CDN patterns
        const isAlabamaImage = src.includes('rolltide.com') ||
    ...
    console.log(`📸 Matched ${headshotList.length} player headshots on Alabama roster\n`);

    // Get Alabama pitchers from database
    const { data: dbPitchers, error: dbError } = await supabase
      .from('cbb_pitchers')
      .select('pitcher_id, name, display_name, headshot')
      .eq('team_id', ALABAMA_TEAM_ID);

    if (dbError) {
      console.error('❌ Error fetching Alabama pitchers:', dbError.message);
      return 0;
    }

    console.log(`🎯 Found ${dbPitchers.length} Alabama pitchers in database\n`);
    ...
    console.error('❌ Error scraping Alabama headshots:', error);
    return 0;
  } finally {
    await browser.close();
  }
...
async function main() {
  console.log('🏀 Alabama Headshot Update\n');
  console.log('='.repeat(50) + '\n');

  const headshotsUpdated = await scrapeHeadshots();

  console.log('\n' + '='.repeat(50));
  console.log(`\n✅ Complete! ${headshotsUpdated} Alabama pitcher headshots updated`);
