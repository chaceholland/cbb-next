...
const HEADSHOTS_DIR = path.join(__dirname, "public/headshots/maryland");
const TEAM_ID = "87";
const CLOUDFRONT_BASE =
  "https://dxbhsrqyrr690.cloudfront.net/sidearm.nextgen.sites/umterps.com";
...
async function main() {
  ...
  for (const pitcher of pitchers) {
    ...
    const ext = (pitcher.headshot || "").toLowerCase().includes(".png")
      ? "png"
      : "jpg";
    const localPath = `/headshots/maryland/${slug}.${ext}`;
    const fullPath = path.join(HEADSHOTS_DIR, `${slug}.${ext}`);
    ...
    // Strip query params and prepend CloudFront base
    const relativePath = pitcher.headshot.replace(/\?.*$/, "");
    const downloadUrl = `${CLOUDFRONT_BASE}${relativePath}`;
