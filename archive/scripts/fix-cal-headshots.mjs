#!/usr/bin/env node
/**
 * Fix California headshots - convert relative paths to CloudFront URLs and download
 */
...
const HEADSHOTS_DIR = path.join(__dirname, "public/headshots/cal");
const TEAM_ID = "65";
const CF_BASE =
  "https://dxbhsrqyrr690.cloudfront.net/sidearm.nextgen.sites/calberk.sidearmsports.com";
...
async function main() {
  ...
  console.log(`Found ${pitchers.length} Cal pitchers\n`);
  ...
  for (const pitcher of pitchers) {
    ...
    const localPath = `/headshots/cal/${slug}.jpg`;
    ...
    if (!pitcher.headshot) {
      console.log(`  ✗ ${pitcher.name}: No headshot URL`);
      errors++;
      continue;
    }

    // Strip query params and build full CloudFront URL
    const relativePath = pitcher.headshot.replace(/\?.*$/, "");
    const fullUrl = `${CF_BASE}${relativePath}`;
