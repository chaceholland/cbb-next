...
const HEADSHOTS_DIR = path.join(__dirname, "public/headshots/iowa");
const TEAM_ID = "167";
...
function decodeImgproxyUrl(imgproxyUrl) {
  // Extract the base64 path at the end (after last /)
  const parts = imgproxyUrl.split("/");
  let b64 = parts[parts.length - 1].replace(/\.jpg$/, "");
  // Add padding
...
async function main() {
  ...
  for (const pitcher of pitchers) {
    ...
    const localPath = `/headshots/iowa/${slug}.jpg`;
