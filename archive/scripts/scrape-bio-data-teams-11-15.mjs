/**
 * Scrape bio data (year, height, weight, hometown, bats/throws) for teams 11-15 with most missing data
 * - Texas Tech
 * - Vanderbilt
 * - South Carolina
 * - Arizona Wildcats
...
const TEAMS = [
  ...
  { name: 'Vanderbilt', team_id: '120', url: 'https://vucommodores.com/sports/baseball/roster' },
  { name: 'South Carolina', team_id: '193', url: 'https://gamecocksonline.com/sports/baseball/roster' },
...
async function scrapeTeamRoster(browser, team) {
  ...
  try {
    ...
    const players = await page.evaluate((teamName) => {
      ...
      // Strategy 1: South Carolina-specific format (#, Name, Position, Height, Weight, Year, B/T, Hometown)
      if (teamName === 'South Carolina') {
...
async function main() {
  console.log('🏀 Bio Data Scraper - Teams 11-15\n');
