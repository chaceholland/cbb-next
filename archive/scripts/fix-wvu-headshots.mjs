...
const HEADSHOTS_DIR = path.join(__dirname, "public/headshots/west-virginia");
const TEAM_ID = "136";
...
// Extract direct CloudFront URL from sidearmdev crop proxy URL
function extractCFUrl(sidearmUrl) {
  const match = sidearmUrl.match(/[?&]url=([^&]+)/);
...
async function main() {
  console.log("\nFixing West Virginia headshots...\n");
  ...
  for (const pitcher of pitchers) {
    ...
    const ext =
      pitcher.headshot && pitcher.headshot.toLowerCase().includes(".png")
        ? "png"
        : "jpg";
    const localPath = `/headshots/west-virginia/${slug}.${ext}`;
    ...
    const downloadUrl = extractCFUrl(pitcher.headshot);
