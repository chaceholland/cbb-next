...
const HEADSHOTS_DIR = path.join(__dirname, "public/headshots/illinois");
const TEAM_ID = "153";
...
async function main() {
  console.log("\nFixing Illinois headshots (full resolution)...\n");
  ...
  for (const pitcher of pitchers) {
    ...
    const ext = (pitcher.headshot || "").toLowerCase().includes(".png")
      ? "png"
      : "jpg";
    const localPath = `/headshots/illinois/${slug}.${ext}`;
    ...
    const fullResUrl = pitcher.headshot.replace(/\?.*$/, "");

    try {
      await downloadImage(fullResUrl, fullPath);
      await supabase
        .from("cbb_pitchers")
        .update({ headshot: localPath })
        .eq("pitcher_id", pitcher.pitcher_id);
