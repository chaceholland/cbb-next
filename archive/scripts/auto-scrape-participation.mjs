...
async function updateScrapeStatus(gameId, status, incrementAttempts = true) {
  ...
  await supabase.from("cbb_games").update(updates).eq("game_id", gameId);
}
...
async function autoScrape(options = {}) {
  ...
  const gamesToScrape = maxGames
    ? missingGames.slice(0, maxGames)
    : missingGames;
  ...
  for (let i = 0; i < gamesToScrape.length; i++) {
    ...
    if (verbose) {
      process.stdout.write(
        `${progress} ${game.away_name} @ ${game.home_name}... `,
      );
    }
    ...
    if (result.error) {
      ...
      if (verbose)
        console.log(
          `⚠️  No data available yet (checked at ${new Date().toLocaleString()})`,
        );
...
async function main() {
  ...
  const options = {
    ...
    delayMs: 300,
    verbose: true,
    ncaaFallbackEnabled: false,
    d1FallbackEnabled: false,
  };
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--days" && args[i + 1]) {
    ...
    } else if (args[i] === "--ncaa-fallback") {
      options.ncaaFallbackEnabled = true;
    } else if (args[i] === "--d1-fallback") {
      options.d1FallbackEnabled = true;
      ...
      console.log(
        "  --d1-fallback     After ESPN pass, try D1Baseball for no-data games",
      );
      console.log(
        "  --ncaa-fallback   After ESPN pass, run NCAA fallback (legacy)",
      );
  ...
  // D1Baseball fallback: attempt to fill remaining no-data games from D1Baseball/StatBroadcast
  let d1Results = null;
  if (options.d1FallbackEnabled) {
    if (options.verbose) {
      console.log(
        "\n⏳ Running D1Baseball fallback for games still missing data...",
      );
    }
    d1Results = await d1BaseballFallback({
  ...
  // NCAA fallback (legacy, kept for reference)
  let ncaaResults = null;
  if (options.ncaaFallbackEnabled) {
    if (options.verbose) {
      console.log("\n⏳ Running NCAA fallback for games still missing data...");
    }
    ncaaResults = await ncaaFallback({
      daysBack: options.daysBack,
      delayMs: 800,
      verbose: options.verbose,
    });
  ...
  const logFile = `scraper-logs/scrape-log-${timestamp}.json`;
