/**
 * Scrape and update Texas A&M pitcher headshots from their official roster page
 */
...
const TEXAS_AM_ROSTER_URL = 'https://12thman.com/sports/baseball/roster';
const TEXAS_AM_TEAM_ID = '123';

async function scrapeHeadshots() {
  console.log('🔍 Scraping Texas A&M headshots...\n');

  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();

  try {
    await page.goto(TEXAS_AM_ROSTER_URL, { waitUntil: 'load', timeout: 30000 });
    ...
    const scraped = await page.evaluate(() => {
      ...
      allImages.forEach(img => {
        ...
        // Check for Texas A&M image CDN patterns
        const isTexasAMImage = src.includes('12thman.com') ||
    ...
    console.log(`📸 Matched ${headshotList.length} player headshots on Texas A&M roster\n`);

    // Get Texas A&M pitchers from database
    const { data: dbPitchers, error: dbError } = await supabase
      .from('cbb_pitchers')
      .select('pitcher_id, name, display_name, headshot')
      .eq('team_id', TEXAS_AM_TEAM_ID);

    if (dbError) {
      console.error('❌ Error fetching Texas A&M pitchers:', dbError.message);
      return 0;
    }

    console.log(`🎯 Found ${dbPitchers.length} Texas A&M pitchers in database\n`);
    ...
    console.error('❌ Error scraping Texas A&M headshots:', error);
    return 0;
  } finally {
    await browser.close();
  }
...
async function main() {
  console.log('🏀 Texas A&M Headshot Update\n');
  console.log('='.repeat(50) + '\n');

  const headshotsUpdated = await scrapeHeadshots();

  console.log('\n' + '='.repeat(50));
  console.log(`\n✅ Complete! ${headshotsUpdated} Texas A&M pitcher headshots updated`);
